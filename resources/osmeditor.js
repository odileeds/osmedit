...
//			S(document).ajax("https://overpass-api.de/api/interpreter?data="+qs,{
				...
				'success':function(d,attr){
					...
					var lastupdate = S(oDOM).find('meta').attr('osm_base').replace('T'," ");
					this.map.attributionControl.addAttribution("Last updated: "+lastupdate);

					// Get nodes
					var features = S(oDOM).find('node');
					...
					for(var i = 0; i < features.length; i++){
						...
						if(!this.nodes[id]){
							this.nodes[id] = {'id':el.attr('id'),'props':{},'popup':'','changedtags':[]};
						}
...
//					console.log(id,popup.options.icon,'BLAH',this.markers[popup.options.icon].background);
